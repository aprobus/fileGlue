...
function glue (filePaths, outputFilePath, callback) {
  ...
  if (!(filePaths && outputFilePath)) {
    ...
    glueFile(filePaths, outputFilePath, callback, glueEmitter);
  }

  return glueEmitter; //Glue is null if callback exists
}

function glueFile (filePaths, outputFilePath, callback, glueEmitter) {
  var readError = null;
...
function tear (filePath, opts, callback) {
  ...
  var outputDir = opts.outputDir || path.dirname(filePath);

  tearFile(filePath, outputDir, path.basename(filePath), tearSize, readSize, callback);
}
